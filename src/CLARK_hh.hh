...
#define MAXREADLEN	100000
#define MAXSCAFLEN	100000
#define MINCFSP		0.75
#define MINGMSP		0.06
#define	MXNMLEN		1000
...
void CLARK<HKMERr>::getObjectsDataComputeFastSpaced(const bool& isfasta, const char* filename, const char* _fileResult)
{
	...
	if (isfasta)
	{
		fdmanager = new FileHandlerA(filename, m_nbCPU, MAXNBREADS);
	}
	else
	{
		fdmanager = new FileHandlerQ(filename, m_nbCPU, MAXNBREADS);
	}
	struct timeval requestStart, requestEnd;
	gettimeofday(&requestStart, NULL);
...
bool CLARK<HKMERr>::getTargetsData(const char* _filesName, vector<string>& _filesHT, vector<string>& _filesHTC, const bool& _creatingkmfiles, const ITYPE& _samplingfactor)
{
	...
	{
		 cerr << "The program did not find the database files for the provided settings and reference sequences. The program will build them." << endl;
	}
	free(cfname);
	free(cfname_s);
	cfname = NULL;
	...
	if (!areHTfilespresent && m_isSpacedLoading)
	{
		...
		cerr << "See instructions in http://clark.cs.ucr.edu/Spaced/"<< endl;
		exit(1);
	}
